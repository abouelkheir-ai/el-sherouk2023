//
//  YStopKastFilterViewController.swift
//  El-Sherouk-Academy
//
...
class YStopKastFilterViewController: UIViewController  ,UITextFieldDelegate  {
        ...
        if let startDate = dateFormatter.date(from: start_date),
           ...
           let termEndDate = dateFormatter.date(from: termEnd) {
            if (startDate < termBeginDate) || (startDate > termEndDate) {
                ...
                var x = ""
                x = "SELECT * FROM y_stop_kast WHERE dates BETWEEN '" + start_date + "' AND '" + end_date + "'"
            ...
            if let navigationController = self.navigationController {
                if let previousViewController = navigationController.viewControllers[navigationController.viewControllers.count - 2] as? YStopKastViewController {
