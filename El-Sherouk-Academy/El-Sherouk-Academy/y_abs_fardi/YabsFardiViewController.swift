...
class YabsFardiViewController: UIViewController , UITableViewDelegate, UITableViewDataSource   {
    var client = SQLClient.sharedInstance()
    var list_yabsFardi: [YabsFardi] = []
    ...
    @IBAction func goFilter(_ sender: Any) {
        ...
        if let vcSecondView = self.storyboard?.instantiateViewController(withIdentifier: "y_abs_fardi_filter") as? YabsFardiFilterViewController {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        print(list_yabsFardi.count)
        
        return list_yabsFardi.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {

        guard let cell = tableView.dequeueReusableCell(withIdentifier: "y_absFardi_cell") as? YabsFardiTableViewCell else { return UITableViewCell() }
        print(indexPath)
        cell.setupCell(dates: list_yabsFardi[indexPath.row].dates ?? "", mostand:  list_yabsFardi[indexPath.row].mostand ?? "", users:  list_yabsFardi[indexPath.row].users ?? "", nCls:  list_yabsFardi[indexPath.row].n_cls ?? "", nStud:  list_yabsFardi[indexPath.row].n_stud ?? "", mada:  list_yabsFardi[indexPath.row].n_sub ?? "", qA:  list_yabsFardi[indexPath.row].q_a ?? 0, Qn:  list_yabsFardi[indexPath.row].q_n ?? 0)
        return cell
    }
                ...
                for table in result {
                    ...
                    for row in table {
                        ...
                        do {
                            ...
                            let instance = try JSONDecoder().decode(YabsFardi.self, from: jsonData)
                            self.list_yabsFardi.append(instance)
                            self._qa += instance.q_a ?? 0
                            self._qn += instance.q_n ?? 0
    ...
    func showAlertWithDetails(q_a: Double, q_n: Double) {
        let  message = """
        غياب باذن: \(q_a)
        غياب بدون اذن:\(q_n)
    ...
    @IBAction func show_total(_ sender: Any) {
        showAlertWithDetails(q_a: _qa, q_n: _qn)
    }
    ...
    @IBAction func search(_ sender: Any) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) {
            ...
            if(self.list_yabsFardi.isEmpty){
