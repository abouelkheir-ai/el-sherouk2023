//
//  NameStudEndFilterViewController.swift
//  El-Sherouk-Academy
//
...
import iOSDropDown
class NameStudEndFilterViewController: UIViewController  ,UITextFieldDelegate  {
...
//                }
    ...
    func configureDropDownLevel() {
        ...
        let level = self.saf_drasiList1
        // Set up the Dropdown
        levelFilter.optionArray = level
        ...
        if let startDate = dateFormatter.date(from: start_date),
           ...
           let termEndDate = dateFormatter.date(from: termEnd) {
            if (startDate < termBeginDate) || (startDate > termEndDate) {
                ...
                var x = ""
                x = "SELECT *, ROW_NUMBER() OVER(ORDER BY n_cls, n_stud) AS nn FROM name_stud_end WHERE d_t BETWEEN '" + start_date + "' AND '" + end_date + "'"
            ...
            if let navigationController = self.navigationController {
                if let previousViewController = navigationController.viewControllers[navigationController.viewControllers.count - 2] as? NameStudEndViewController {
