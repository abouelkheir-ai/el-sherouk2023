...
import iOSDropDown
class TiradViewController: UIViewController , UITableViewDelegate, UITableViewDataSource  ,UITextFieldDelegate  {
    
    var client = SQLClient.sharedInstance()
    var list_tirad: [Tirad] = []
...
//            }
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        return list_tirad.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {

        guard let cell = tableView.dequeueReusableCell(withIdentifier: "t_irad_cell") as? TiradTableViewCell else { return UITableViewCell() }
        print(indexPath)
        

        cell.setupCell(dates: list_tirad[indexPath.row].dates ?? "", nohMostand:list_tirad[indexPath.row].noh_mostand ?? "", mostand: list_tirad[indexPath.row].mostand ?? "", users: list_tirad[indexPath.row].users ?? "", nStud: list_tirad[indexPath.row].n_stud ?? "", inv: list_tirad[indexPath.row].inv ?? 0, notes: list_tirad[indexPath.row].notes ?? "", cash: list_tirad[indexPath.row].cash ?? 0, bank: list_tirad[indexPath.row].bank ?? 0, t: list_tirad[indexPath.row].t ?? 0)
        return cell
    }
            ...
            client.execute(self.query) { results in
                ...
                for table in result {
                    ...
                    for row in table {
                        ...
                        do {
                            let jsonData = try JSONSerialization.data(withJSONObject: row)
                            let instance = try JSONDecoder().decode(Tirad.self, from: jsonData)
                            self.list_tirad.append(instance)
        ...
        if let startDate = dateFormatter.date(from: start_date),
           ...
           let termEndDate = dateFormatter.date(from: termEnd) {
            if (startDate < termBeginDate) || (startDate > termEndDate) {
                ...
                var x  = ""
                x = "SELECT dates , noh_mostand, mostand,users,n_stud,inv,notes,cash,bank,t FROM t_irad WHERE dates BETWEEN '" + start_date + "' AND '" + end_date + "'"
