//
//  NameStudSSViewController.swift
//  El-Sherouk-Academy
//
...
class NameStudSSViewController: UIViewController , UITableViewDelegate, UITableViewDataSource   {
    var client = SQLClient.sharedInstance()
    var listNameStudSS: [NameStudSS] = []
    ...
    @IBAction func goFilter(_ sender: Any) {
        ...
        if let vcSecondView = self.storyboard?.instantiateViewController(withIdentifier: "name_stud_ss_filter") as? NameStudSSFilterViewController {
            ...
            print("Error: Unable to instantiate view controller with identifier 'name_stud_ss_filter'")
        }
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        print(listNameStudSS.count)
        _q = listNameStudSS.count
        
        return listNameStudSS.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {

        guard let cell = tableView.dequeueReusableCell(withIdentifier: "name_stud_ss_cell") as? NameStudSSTableViewCell else { return UITableViewCell() }
        print(indexPath)


        cell.setupCell(n_cls: listNameStudSS[indexPath.row].n_cls ?? "", n_stud: listNameStudSS[indexPath.row].n_stud ?? "", n_moz: listNameStudSS[indexPath.row].n_moz ?? "", dates: listNameStudSS[indexPath.row].dates ?? "", n_school: listNameStudSS[indexPath.row].n_school ?? "")
        return cell
    }
                ...
                for table in result {
                    ...
                    for row in table {
                        ...
                        do {
                            ...
                            let instance = try JSONDecoder().decode(NameStudSS.self, from: jsonData)
                            self.listNameStudSS.append(instance)
    ...
    @IBAction func search(_ sender: Any) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) {
            ...
            if(self.listNameStudSS.isEmpty){
