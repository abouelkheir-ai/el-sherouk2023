...
class YclrMozViewController: UIViewController , UITableViewDelegate, UITableViewDataSource   {
    var client = SQLClient.sharedInstance()
    var list_yclrMoz: [YclrMoz] = []
    ...
    @IBAction func goFilter(_ sender: Any) {
        ...
        if let vcSecondView = self.storyboard?.instantiateViewController(withIdentifier: "yclrFilter_ui") as? YClrMozFilterViewController {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        print(list_yclrMoz.count)
        _q = list_yclrMoz.count
        return list_yclrMoz.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {

        guard let cell = tableView.dequeueReusableCell(withIdentifier: "y_clr_moz_cell") as? YClrMozTableViewCell else { return UITableViewCell() }
        print(indexPath)


        cell.setupCell(dates: list_yclrMoz[indexPath.row].dates ?? "", mostand: list_yclrMoz[indexPath.row].mostand ?? "", nMoz: list_yclrMoz[indexPath.row].n_moz ?? "", nCont: list_yclrMoz[indexPath.row].no_cont ?? 0, nWaz: list_yclrMoz[indexPath.row].n_waz ?? "", notes: list_yclrMoz[indexPath.row].notes ?? "")
        return cell
    }
                ...
                for table in result {
                    ...
                    for row in table {
                        ...
                        do {
                            ...
                            let instance = try JSONDecoder().decode(YclrMoz.self, from: jsonData)
                            self.list_yclrMoz.append(instance)
