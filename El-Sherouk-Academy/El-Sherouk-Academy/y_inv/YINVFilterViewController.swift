//
//  YINVFilterViewController.swift
//  El-Sherouk-Academy
//
...
import iOSDropDown
class YINVFilterViewController: UIViewController  ,UITextFieldDelegate  {
    ...
    @IBOutlet weak var scrollView: UIScrollView! // Add this outlet for the UIScrollView
...
//        }
    ...
    func configureDropDownLocation() {
        ...
        let locationList = self.locationList1
            // Set up the Dropdown
            loacationFilter.optionArray = locationList
        ...
        if let startDate = dateFormatter.date(from: start_date),
           ...
           let termEndDate = dateFormatter.date(from: termEnd) {
            if (startDate < termBeginDate) || (startDate > termEndDate) {
                ...
                var x = ""
                x = "SELECT * FROM y_inv WHERE dates BETWEEN '" + start_date + "' AND '" + end_date + "'"
            ...
            if let navigationController = self.navigationController {
                if let previousViewController = navigationController.viewControllers[navigationController.viewControllers.count - 2] as? YInvViewController {
