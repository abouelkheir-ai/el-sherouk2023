...
class StBankViewController: UIViewController , UITableViewDelegate, UITableViewDataSource   {
    var client = SQLClient.sharedInstance()
    var list_stBank: [StBank] = []
    ...
    @IBAction func goFilter(_ sender: Any) {
        ...
        if let vcSecondView = self.storyboard?.instantiateViewController(withIdentifier: "st_bank_filter") as? StBankFilterViewController {
            ...
            print("Error: Unable to instantiate view controller with identifier 'st_bank_filter'")
        }
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        print(list_stBank.count)
        
        return list_stBank.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {

        guard let cell = tableView.dequeueReusableCell(withIdentifier: "st_bank_cell") as? StBankTableViewCell else { return UITableViewCell() }
        print(indexPath)

        cell.setupCell(dates: list_stBank[indexPath.row].dates ?? "", noh_mostand: list_stBank[indexPath.row].noh_mostand ?? "", mostand: list_stBank[indexPath.row].mostand ?? "", hh: list_stBank[indexPath.row].hh ?? "", notes: list_stBank[indexPath.row].notes ?? "", madin: list_stBank[indexPath.row].madin ?? 0, dain: list_stBank[indexPath.row].dain ?? 0, r: list_stBank[indexPath.row].r ?? 0)
                ...
                for table in result {
                    ...
                    for row in table {
                        ...
                        do {
                            ...
                            let instance = try JSONDecoder().decode(StBank.self, from: jsonData)
                            self.list_stBank.append(instance)
