//
//  StBankFilterViewController.swift
//  El-Sherouk-Academy
//
...
import iOSDropDown
class StBankFilterViewController: UIViewController  ,UITextFieldDelegate  {
    ...
    let pickerBankName = UIPickerView()
...
//            }
    ...
    func searchForBankName(_searchText: String) {
          ...
          bank_name.removeAll()
        let query2 = " SELECT  TOP 10 n_acc , id_acc FROM name_bank  where n_acc like '%\(_searchText)%'"
            ...
            if (startDate < termBeginDate) || (startDate > termEndDate) {
                ...
                if switchOn {
                    ...
                    connectCheckLr(_lrQuery: queryLr) { lR in
                        self.query = "SELECT ISNULL(dates, '" + self.start_date + "') AS dates, noh_mostand, cod_mostand, mostand, hh, notes, madin, dain, user_s, td_s, user_e, td_e, id_trm, ISNULL ((SELECT SUM(r) AS r FROM st_bank_uc AS st_bank_uc_1 WHERE (st_bank_uc.id_acc = id_acc) AND (st_bank_uc.td00 >= td00)), '" + String(lR) + "') AS r FROM st_bank_uc WHERE (dates BETWEEN '" + self.start_date + "' AND '" + self.end_date + "') AND (id_acc = '" + String(self.bank_id) + "') OR ((id_acc IS NULL) and (ISNULL ((SELECT SUM(r) AS r FROM st_bank_uc AS st_bank_uc_1 WHERE (st_bank_uc.id_acc = id_acc) AND (st_bank_uc.td00 >= td00)), '" + String(lR) + "') <> 0)) ORDER BY td00"
                        self.performQuery()
                    }
                } else {
                    let queryLr = "SELECT SUM(rr) AS lr FROM bal_acc_s WHERE d < '" + start_date + "' AND id_acc = '" + String(bank_id) + "'"
                    connectCheckLr(_lrQuery: queryLr) { lR in
                        self.query = "SELECT ISNULL(dates, '" + self.start_date + "') AS dates, noh_mostand, cod_mostand, mostand, hh, notes, madin, dain, user_s, td_s, user_e, td_e, id_trm, ISNULL(r, '" + String(lR) + "') AS r FROM st_bank WHERE (dates Between '" + self.start_date + "' AND '" + self.end_date + "' AND id_acc = '" + String(self.bank_id) + "') OR ((id_acc IS NULL) AND (ISNULL(r, '" + String(lR) + "') <> '0')) ORDER BY td00 ASC"
                        self.performQuery()
                    }
                }
            }
    ...
    func performQuery() {
        if let navigationController = self.navigationController {
            if let previousViewController = navigationController.viewControllers[navigationController.viewControllers.count - 2] as? StBankViewController {
