//
//  YattabsGroupFilterViewController.swift
//  El-Sherouk-Academy
//
...
import iOSDropDown
class YattabsGroupFilterViewController: UIViewController  ,UITextFieldDelegate  {
    ...
    var statusList = ["حضور","غياب","بدون تحديد"]
    ...
    override func viewDidLoad() {
        ...
        end_date = dateString
        statusFilter.optionArray = ["حضور","غياب","بدون تحديد"]
...
//            }
    ...
    func configureDropDownStudent() {
        ...
        let studentList = self.stud_name1
            // Set up the Dropdown
            studentFilter.optionArray = studentList
        ...
        if let startDate = dateFormatter.date(from: start_date),
           ...
           let termEndDate = dateFormatter.date(from: termEnd) {
            if (startDate < termBeginDate) || (startDate > termEndDate) {
                ...
                var x = ""
                x = "SELECT * FROM y_att_abs_group WHERE dates BETWEEN '" + start_date + "' AND '" + end_date + "'"
            ...
            if let navigationController = self.navigationController {
                if let previousViewController = navigationController.viewControllers[navigationController.viewControllers.count - 2] as? YattabsGroupViewController {
