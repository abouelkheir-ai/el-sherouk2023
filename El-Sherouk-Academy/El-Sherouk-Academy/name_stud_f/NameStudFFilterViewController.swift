//
//  NameStudFFilterViewController.swift
//  El-Sherouk-Academy
//
...
import iOSDropDown
class NameStudFFilterViewController: UIViewController  ,UITextFieldDelegate  {
...
//                    overrideUserInterfaceStyle = .dark // or .dark based on your preference
//                }
    }
        ...
        if let startDate = dateFormatter.date(from: start_date),
           ...
           let termEndDate = dateFormatter.date(from: termEnd) {
            if (startDate < termBeginDate) || (startDate > termEndDate) {
                ...
                x = "SELECT n_cls, n_stud, id_f_stud, gen, tel_m, n_school, n_gov, users, d_f, d_t, user_s, td_s, user_e, td_e, mostand, cod_mostand, id_trm, ROW_NUMBER() OVER(ORDER BY " + y + "n_cls) AS nn FROM name_stud_f WHERE d_f <= '" + start_date + "' AND d_t >= '" + end_date + "'"
            ...
            if let navigationController = self.navigationController {
                if let previousViewController = navigationController.viewControllers[navigationController.viewControllers.count - 2] as? NameStudFViewController {
