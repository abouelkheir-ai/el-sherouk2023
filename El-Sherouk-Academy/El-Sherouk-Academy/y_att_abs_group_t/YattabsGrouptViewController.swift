//
//  YattabsGroupTViewController.swift
//  El-Sherouk-Academy
//
...
import iOSDropDown
class YattabsGrouptViewController: UIViewController ,UITextFieldDelegate, UITableViewDelegate, UITableViewDataSource {
    ...
    var list_YattGroupT: [YattGorupT] = []
...
//        }
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        print(list_YattGroupT.count)
        return list_YattGroupT.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "y_gorupT_cell") as? yattabsGroupTTableViewCell else { return UITableViewCell() }
        print(indexPath)
        

        cell.setupCell(n_stud: list_YattGroupT[indexPath.row].n_stud ?? "", sec1: list_YattGroupT[indexPath.row].sect1 ?? "", sec2: list_YattGroupT[indexPath.row].sect2 ?? "", sec3: list_YattGroupT[indexPath.row].sect3 ?? "", sec4: list_YattGroupT[indexPath.row].sect4 ?? "", sec5: list_YattGroupT[indexPath.row].sect5 ?? "")
        return cell
    }
    func connect()  {
        print(query)
        ...
        self.list_YattGroupT.removeAll()
            ...
            for table in result {
                ...
                for row in table {
                    ...
                    do {
                        ...
                        let instance = try JSONDecoder().decode(YattGorupT.self, from: jsonData)
                        self.list_YattGroupT.append(instance)
                        self._c1 += Int(instance.c1 ?? "0") ?? 0
                        self._c2 += Int(instance.c2 ?? "0") ?? 0
                        self._c3 += Int(instance.c3 ?? "0") ?? 0
                        self._c4 += Int(instance.c4 ?? "0") ?? 0
                        self._c5 += Int(instance.c5 ?? "0") ?? 0
    ...
    func showAlertWithDetails(c1: Int,c2: Int,c3: Int,c4: Int,c5: Int) {
        let  message = """
        حصص الحضور: \(c1)
        حصص الغياب: \(c2)
        غياب بدون اذن: \(c3)
    ...
    showAlertWithDetails(c1: _c1, c2: _c2, c3: _c3, c4: _c4, c5: _c5)
    }
    ...
    @IBAction func search(_ sender: Any) {
        ...
        if (startDate < termBeginDate) || (startDate > termEndDate) {
            ...
            if groupPickFilter.text == "" || idGroup == 0 {
                ...
                query = "SELECT *, ROW_NUMBER() OVER(ORDER BY n_stud) AS nn FROM y_att_abs_group_t WHERE dates = '" + start_date + "' AND id_group = '" + String(idGroup) + "' ORDER BY n_stud ASC"
            }
