//
//  WrkOrdFFilterViewController.swift
//  El-Sherouk-Academy
//
...
import iOSDropDown
class WrkOrdFFilterViewController: UIViewController  ,UITextFieldDelegate  {
        ...
        if let startDate = dateFormatter.date(from: start_date),
           ...
           let termEndDate = dateFormatter.date(from: termEnd) {
            if (startDate < termBeginDate) || (startDate > termEndDate) {
                ...
                if !switchOn {
                    print(switchOn)
                    x = "SELECT * FROM wrk_ord_f WHERE d_f <= '" + start_date + "' AND d_t >= '" + end_date + "'"
                }else{
                    x = "SELECT * FROM wrk_ord_f WHERE dates BETWEEN '" + start_date + "' AND '" + end_date + "'"
            ...
            if let navigationController = self.navigationController {
                if let previousViewController = navigationController.viewControllers[navigationController.viewControllers.count - 2] as? WrkOrdFViewController {
