...
class NameStudGViewController: UIViewController , UITableViewDelegate, UITableViewDataSource   {
    var client = SQLClient.sharedInstance()
    var listNameStudG: [NameStudG] = []
    ...
    @IBAction func goFilter(_ sender: Any) {
        ...
        if let vcSecondView = self.storyboard?.instantiateViewController(withIdentifier: "name_stud_g_filter") as? NameStudGFilterViewController {
            ...
            print("Error: Unable to instantiate view controller with identifier 'name_stud_g_filter'")
        }
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        print(listNameStudG.count)
        _q = listNameStudG.count
        
        return listNameStudG.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {

        guard let cell = tableView.dequeueReusableCell(withIdentifier: "name_stud_g_cell") as? NameStudGTableViewCell else { return UITableViewCell() }
        print(indexPath)


        cell.setupCell(nCls: listNameStudG[indexPath.row].n_cls ?? "", nStud: listNameStudG[indexPath.row].n_stud ?? "", gen: listNameStudG[indexPath.row].gen ?? "", nShcool: listNameStudG[indexPath.row].n_school ?? "", nGov: listNameStudG[indexPath.row].n_gov ?? "", users: listNameStudG[indexPath.row].users ?? "", df: listNameStudG[indexPath.row].d_f ?? "", dt: listNameStudG[indexPath.row].d_t ?? "")
        return cell
    }
                ...
                for table in result {
                    ...
                    for row in table {
                        ...
                        do {
                            ...
                            let instance = try JSONDecoder().decode(NameStudG.self, from: jsonData)
                            self.listNameStudG.append(instance)
