//
//  BalKastFilterViewController.swift
//  El-Sherouk-Academy
//
...
import iOSDropDown
class BalKastFilterViewController: UIViewController  ,UITextFieldDelegate  {
    ...
    override func viewDidLoad() {
        ...
        genFilter.backgroundColor = .darkGray
        userFilter.backgroundColor = .darkGray
        levelFilter.backgroundColor = .darkGray
        fatherFilter.backgroundColor = .darkGray
        invoiceFilter.backgroundColor = .darkGray
        employeeFilter.backgroundColor = .darkGray
...
//                }
    ...
    func searchForFather(_searchText: String) {
        ...
        let query2 = "SELECT TOP 50 n_f_stud , id_f_stud FROM name_f_stud WHERE n_f_stud LIKE '%\(_searchText)%'"
        ...
        if let startDate = dateFormatter.date(from: start_date),
           ...
           let termEndDate = dateFormatter.date(from: termEnd) {
            if (startDate < termBeginDate) || (startDate > termEndDate) {
                ...
                var x = ""
                x = "SELECT *, ROW_NUMBER() OVER(ORDER BY " + y + "d_kast) AS nn FROM bal_kast WHERE d_kast BETWEEN '" + start_date + "' AND '" + end_date + "'"
            ...
            if let navigationController = self.navigationController {
                if let previousViewController = navigationController.viewControllers[navigationController.viewControllers.count - 2] as? BalKstViewController {
