...
class StBankTViewController: UIViewController , UITableViewDelegate, UITableViewDataSource   {
    var client = SQLClient.sharedInstance()
    var list_stBankt: [StBankT] = []
    ...
    @IBAction func goFilter(_ sender: Any) {
        ...
        if let vcSecondView = self.storyboard?.instantiateViewController(withIdentifier: "st_bank_t_filter") as? STbankTFilterViewController {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        print(list_stBankt.count)
        
        return list_stBankt.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {

        guard let cell = tableView.dequeueReusableCell(withIdentifier: "st_bank_t_cell") as? StBankTTableViewCell else { return UITableViewCell() }
        print(indexPath)


        cell.setupCell(dates: list_stBankt[indexPath.row].dates ?? "", madin: list_stBankt[indexPath.row].madin ?? 0, dain: list_stBankt[indexPath.row].dain ?? 0, r: list_stBankt[indexPath.row].r ?? 0)
                ...
                for table in result {
                    ...
                    for row in table {
                        ...
                        do {
                            ...
                            let instance = try JSONDecoder().decode(StBankT.self, from: jsonData)
                            self.list_stBankt.append(instance)
