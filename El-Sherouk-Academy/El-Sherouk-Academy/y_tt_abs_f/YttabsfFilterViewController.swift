//
//  YttabsfFilterViewController.swift
//  El-Sherouk-Academy
//
...
import iOSDropDown
class YttabsfFilterViewController: UIViewController  ,UITextFieldDelegate  {
    ...
    @IBOutlet weak var statusFilter: DropDown!
    ...
    override func viewDidLoad() {
        ...
        statusFilter.optionArray = ["غياب","حضور"]
        statusFilter.didSelect { (selectedText , index ,id) in
            self.statusFilter.text = "Selected String: \(selectedText) \n index: \(index)"
        }
        ...
        statusFilter.backgroundColor = .white
        ...
        statusFilter.overrideUserInterfaceStyle = .light // Set this to dark for dark mode
...
//                
//                statusFilter.backgroundColor = isDarkMode ? .darkGray : .white
//                statusFilter.textColor = isDarkMode ? .white : .black
//
//             
...
//            }
    ...
    func configureDropDownStudent() {
            ...
            let student = self.stud_name1
                // Set up the Dropdown
                statusFilter.optionArray = student

        statusFilter.didSelect { (selectedItem, index, id) in
                    print("Selected item: \(selectedItem) at index \(index)")
        ...
        if let startDate = dateFormatter.date(from: start_date),
           ...
           let termEndDate = dateFormatter.date(from: termEnd) {
            if (startDate < termBeginDate) || (startDate > termEndDate) {
                ...
                var x = ""
                x = "SELECT dates,mostand,att_abs,users,n_cls,n_stud,n_moz,q FROM y_att_abs_f WHERE dates BETWEEN '" + start_date + "' AND '" + end_date + "'"
                ...
                if statusFilter.text != "" {
                    x = x + " AND att_abs = '" + (statusFilter.text ?? "") + "'"
            ...
            if let navigationController = self.navigationController {
                if let previousViewController = navigationController.viewControllers[navigationController.viewControllers.count - 2] as? YttabsFViewControllerr {
